...
```{r}
upperListOfFiles <- toupper(listOfFiles)
ansLoc<-str_which(upperListOfFiles,"ANS")
studentAnswers <- answerCollection[-ansLoc]
correctAnswers <- answerCollection[[ansLoc]]
...
for(i in 1:length(studentAnswers)){
  ...
  result_i <- map2_int(
            ...
            identical)
  newResult<-c(
    studentAnswers[[i]]$name,studentAnswers[[i]]$id,
    result_i
  )
  results <- rbind(results, newResult)
}

#result_iLength <- length(result_i)
#results <- results[,1:result_iLength]
colnames(results)[1:2] <- c("姓名","學號")

# 
if(wsName %in% gs_ws_ls(gsCorrected)){
  gs_edit_cells(gsCorrected,ws=wsName,
                input=results, col_names = TRUE,
                anchor="A1")
