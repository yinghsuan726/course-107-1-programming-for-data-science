...
```{r ans11}
  ...
  classData$`公-10-06` <- str_replace_all(classData$`公-10-06`, "1", "有看公告") 
  classData$`公-10-06` <- str_replace_all(classData$`公-10-06`, "0", "沒看公告")
  classData$`公-10-06`
```
...
library(stringr)
sum(classData$`公-10-06` == "有看公告") -> cc
sum(classData$`公-10-06` == "有看公告" & classData$`練2018-10-17-1` == 1) -> dd
dd/cc
```
...
```{r ans14}
sum(classData$`公-10-06` == "沒看公告") -> ee
sum(classData$`公-10-06` == "沒看公告" & classData$`練2018-10-17-1` == 1) -> ff
ff/ee
```
...
```{r ans23}
hwData$`early` <- difftime(deadline, hwData$`Timestamp`, "Asia/Taipei", "days")
```
...
```{r ans25}
library(dplyr)
hwData <- slice(hwData,1:81)
table(hwData[hwData$`作業代碼`=="hw1",3])
```
