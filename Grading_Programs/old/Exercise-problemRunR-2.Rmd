...
dirSet<-paste0(params$dirSet,"/problem")
...
load(paste0("../",params$hwcode,"/problemTryCatchTrack.Rda"))
rm(list=dataObjects)
...
listOfRs<-list_files_with_exts(problemDir,"R")
library(stringr)
...
# 記錄可儲存catchList的名單
oklistOfSavedTryCatchFiles <- list_files_with_exts(problemDir,"Rda")
...
dataSubmit$shortHWname <- dataSubmit$新檔名 %>%
  str_replace(paste0(problemDir,"/"),"") %>% 
  str_replace(".Rmd","") 
dataSubmit %>% select(-contains("ans")) %>% 
  left_join(
