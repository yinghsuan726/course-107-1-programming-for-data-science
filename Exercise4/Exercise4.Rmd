...
## 3.1
將所有變數的class修正正確。(hint: 使用`mutate_at()`)
```{r ans31}
```

## 3.2
每一國家對其貨幣匯率有兩種標示方式，以台灣看新台幣為例：一種為direct quote，標示為USD/NTD，代表一塊新台幣（NTD）可以換多少美元（USD)； 另一種為indirect quote，標示為NTD/USD，代表一塊美金（USD）可以換多少新台幣（NTD）--兩者互為倒數關係，即若NTD/USD=40，則USD/NTD=1/40。不同國家貨幣的匯率習慣標示皆有所不同，請將資料中的貨幣都轉成"XXX/USD"，即"USD"在分母。(hint: 使用`mutate_at()`)

```{r ans32}
```

## 3.3
將上題轉換的匯率變數名稱中英文拿掉，如原名為"英鎊USD/GBP"則需改成"英鎊"。
