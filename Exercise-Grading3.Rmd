...
姓名	學號	新檔名	cannotPurl	cannotSourceR	ans11	ans12	ans2	上傳檔正常

```{r upload result to GS}
#gsMapping %>% gs_read(ws=wsName) -> fileMapping

dataSubmit %>% select(姓名,學號,新檔名,cannotPurl,cannotSourceR,cannotRdata,starts_with("ans")) %>%
  mutate(學號=as.character(學號))  -> mergedResults

mergedResults %>% mutate(
  上傳檔正常=(!cannotPurl & !cannotSourceR & !cannotRdata)
  ) -> mergedResults

publicResults <-
  mergedResults %>% select(
    -姓名,-新檔名,-cannotPurl, -cannotSourceR, -cannotRdata
  )
