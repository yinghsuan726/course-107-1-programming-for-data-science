...
# choose corrected
listOfcRMDS <- listOfRMDs %>% str_subset("(-C\\.RMD)")
...
listOfRs <- file.path(
  workingPath,
  {basename(listOfcRMDS) %>%
      str_replace("(RMD)","R")}
)
#purrr::map(1:length(listOfcRMDS),knitr::purl) -> purlInfo


for(i in seq_along(listOfcRMDS)){
 knitr::purl(listOfcRMDS[i],output=listOfRs[i]) 
}


dataSubmitToProcess <- data.frame(
  listOfcRMDS=listOfcRMDS,
  canProduceR=T,
  shortNames=(listOfcRMDS %>% basename %>%
                str_replace("\\.(RMD)",""))
)
```
...
markOff(listOfRsNoAns,params,chunkSheetFile,ansRdaFile)
```
