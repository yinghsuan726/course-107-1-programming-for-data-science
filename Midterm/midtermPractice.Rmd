...
```{r ans1b5}
which(hwGrades$學號=="4104d245c7")->a
hwGrades[a,]$Exercise3
```
...
```{r ans12}
str_detect(classSurvey$課外活動,"團隊活動")->a

classSurvey$課外活動%>%str_detect("團隊活動")%>%sum
```
...
```{r ans133}
subsample$課外活動%>%str_detect("團隊活動")%>%table/nrow(subsample)
```
...
```{r ans14}
3-(hwGrades$hw1%>%is.na+
hwGrades$hw2%>%is.na+
hwGrades$Exercise3%>%is.na)->hwGrades$交作業次數
```
...
```{r ans15}
rowSums(hwGrades[,c("hw1","hw2","Exercise3")],na.rm=TRUE)/3->hwGrades$平均成績
...
```{r data2}
hwSubmit <- ("https://raw.githubusercontent.com/tpemartin/github-data/master/midterm-data/hwSubmit.csv")
```
...
hwSubmit[(hwSubmit$作業代碼=="Exercise3"),]$Timestamp->a
deadline-a
```
